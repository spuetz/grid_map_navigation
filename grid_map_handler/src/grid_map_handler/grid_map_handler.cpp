...
namespace grid_map_handler{
  ...
  GridMapHandler::GridMapHandler(const std::string &fname, double res, double local_map_length, double inflation_radius,
                     bool use_polygon, const TFPtr &tf, grid_map::GridMap &global_grid_map,
...
void GridMapHandler::mergeInflationLayers() { //TODO only rewrite the affected regions (faster?)
  ...
  tf2::Stamped<tf::Point> origin(tf::Point(0, 0, 0), ros::Time(0), "base_footprint");
  tf2::Stamped<tf::Point> robot_pos;
  ...
  while(merge_ && ros::ok()) {
    ...
    try {
      robot_pos = tf_->transform(origin, "map", ros::Time(0), "map", ros::Duration(1.0));
    }
...
void GridMapHandler::tfRotationBroadcast() {
  while(broadcast_ && ros::ok()){
    ...
    try {
      transform = tf_->transform(transform, "map", ros::Time(0), "map", ros::Duration(1.0));
    }
    catch (tf::TransformException &ex) {
    ...
    tf2::Stamped<tf::Transform> rot;
    tf2::convert(transform, rot);
    tf::StampedTransform send_tf;
    send_tf.setIdentity();
    send_tf.setRotation(rot.getRotation().inverse());
    send_tf.frame_id_ = "base_footprint";
    send_tf.child_frame_id_ = "base_rotation";
    send_tf.stamp_ = transform.header.stamp;
...
//    ros::Time begin = ros::Time::now();
    ...
    // transform
    try {
        out_msg = tf_->transform(*laserMessage, local_map_.getFrameId(), ros::Duration(1.0));
        projector_.transformLaserScanToPointCloud(local_map_.getFrameId(), out_msg, cloud, *tf_);
    }
    catch (tf::TransformException &ex) {
...
void GridMapHandler::move(){
  ...
  while(move_ && ros::ok()){
   ...
   if(first_pos_call_){
     ...
     tf_moved.setIdentity();
     tf_moved.frame_id_ = local_map_.getFrameId();
     tf_moved.stamp_ = last_moved_;
     try {
       tf_moved = tf_->transform(tf_moved, local_map_.getFrameId(), now, "map");
     }
     catch (tf::TransformException &ex) {
...
int main(int argc, char** argv){
  ...
  boost::shared_ptr<tf2_ros::Buffer> tf_ptr = boost::make_shared<tf2_ros::Buffer>();

  grid_map_handler::GridMapHandler grid_map_handler_(argv[1], atof(argv[2]), 0, 0, false, tf_ptr, grid_map_1, grid_map_2);
